    ...
    public class StageManager : NetworkBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (IsServer)
            {
                ...
                m_GameTimer = m_DelayedStartTime;
        ...
        private void UpdateGameTimer()
        {
            ...
            if (!HasGameStarted() && m_GameTimer > 0.0f)
            {
                m_GameTimer -= Time.deltaTime;
                
                if (IsServer && m_GameTimer <= 0.0f) // Only the server should be updating this
                {
                ...
                if (m_GameTimer > 0.1f) { gameTimerText.SetText("{00}", Mathf.FloorToInt(m_GameTimer)); }
            }
        }
